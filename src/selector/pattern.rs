		...
		if let Some(n) = n {
			...
			match n {
				_ if n < 0 => {
					...
					// -2n + 4
					let mut end_loop = ((index - 1) / -n) as usize;
					// n * 'n' + index >= 1
					let start_loop = ((total as isize - 1 - index) / n) as usize;
					let start_loop = if start_loop > 0 { start_loop } else { 0 };
					// start > end, -2n + 4,
					// total: 1, start_loop = 2, end_loop = 1
					// total: 2, start_loop = 1, end_loop = 1
					// total: 4, start_loop = 0, end_loop = 1
					if start_loop > end_loop {
						return vec![];
					}
					let mut allow_indexs = Vec::with_capacity((end_loop - start_loop + 1) as usize);
					for i in start_loop..=end_loop {
						let cur_index = (i as isize * n + index) as usize;
						allow_indexs.push(cur_index);
					}
					return allow_indexs;
				}
				0 => {
